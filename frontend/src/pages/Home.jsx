...
import { fetchTrending, fetchNetflixOriginals, fetchTopRated, fetchActionMovies } from '../utils/tmdb';
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [trendingRes, originalsRes, topRatedRes, actionRes] = await Promise.all([
          fetchTrending(),
  ...
  return (
    <div className="home fade-in">
      ...
      <div className="rows-container" style={{ marginTop: '-100px', position: 'relative', zIndex: 10 }}>
        ...
        <MovieRow title="Action Thillers" movies={action} />
      </div>
    </div>
  );
};
